...
- **`prosody-data-relocate` must run before `prosody-chown`.** `stable-11146-2` moved `data_path` to `/var/lib/prosody/data`, and upstream migrates only from the `/config` seed — so accounts written by earlier releases stay at the root of the storage volume, invisible to the new config. The oneshot moves them once and no-ops when `data/` is already populated; running it first lets `chown -R` cover the directory it creates.
